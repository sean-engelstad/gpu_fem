...
template <typename T>
class GPUdiagmat {
    // block-Jacobi/diagonal BSR preconditioner for multi-GPU vectors
   ...
   public:
    GPUdiagmat(cublasHandle_t &cublasHandle_, cusparseHandle_t &cusparseHandle_, int *h_rowp_,
        ...
        tmp = new GPUvec<T>(cublasHandle, ngpus, N, block_dim, debug);
    ...
    void factor() {
        for (int g = 0; g < ngpus; g++) {
            ...
            CUSPARSE::perform_ilu0_factorization(cusparseHandle, descr_L, descr_U, info_L, info_U,
                                                 &pBuffer[g], mb, nnzb, block_dim, d_diag_vals[g],
    ...
    void solve(GPUvec<T> *x, GPUvec<T> *y) {
        for (int g = 0; g < ngpus; g++) {
            ...
            CHECK_CUSPARSE(cusparseDbsrsv2_solve(
                ...
                d_diag_cols[g], block_dim, info_L, loc_x, loc_tmp, policy_L, pBuffer[g]));
            CHECK_CUSPARSE(cusparseDbsrsv2_solve(
                cusparseHandle, dir, trans_U, mb, nnzb, &a, descr_U, d_diag_vals[g], d_diag_rowp[g],
                d_diag_cols[g], block_dim, info_U, loc_tmp, loc_y, policy_U, pBuffer[g]));
        }
    }
    ...
    int **h_diag_rowp = nullptr, **h_diag_cols = nullptr;
