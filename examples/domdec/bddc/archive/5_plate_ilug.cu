...
int main(int argc, char **argv) {
    ...
    using KIGMRES = GMRESSolver<T, GRID, 100>;
    using GamGMRES = MatrixFreeGMRESSolver<T, BDDC, GRID, 100>; // BDDC is the operator and preconditioner
    ...
    auto i_krylov = new KIGMRES(cublasHandle, cusparseHandle, grid, i_solver, ki_opts, 300, bddc->getInvars());
    ...
    auto *gam_solver =
        new GamGMRES(cublasHandle, cusparseHandle, grid, bddc, bddc, opts, 300, bddc->getLambdaSize());
