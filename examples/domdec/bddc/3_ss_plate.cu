...
int main(int argc, char **argv) {
    ...
    auto assembler = createPlateAssembler<Assembler>(
        nxe, nye, Lx, Ly, E, nu, thick, rho, ys, nxe_per_comp, nye_per_comp);
    ...
    bool track_dirichlet = false; // shouldn't track it for general BCs
    bddc->setup_structured_subdomains(nxe, nye, nxs, nys, close_hoop, track_dirichlet);
    ...
    if (compare_direct) {
        ...
        auto assembler2 = createPlateAssembler<Assembler>(
            nxe, nye, Lx, Ly, E, nu, thick, rho, ys, nxe_per_comp, nye_per_comp);
