...
int main(void) {
  ...
  double qorder_p = 0.5; // lower p value
  ...
  write_to_csv<int>(orig_rowPtr, nnodes + 1, "csv/orig_rowPtr.csv");
  write_to_csv<int>(orig_colPtr, orig_nnzb, "csv/orig_colPtr.csv");
  ...
  if (perform_rcm) {
    ...
    int root_node = 0;
    int num_rcm_iters = 1;
    TacsComputeRCMOrder(nnodes, orig_rowPtr, orig_colPtr, _new_perm, root_node,
                        num_rcm_iters);
  ...
  if (perform_qordering) {
    ...
    std::vector<int> q_perm(perm,
                            perm + nnodes); // TODO : define it relative to perm
  ...
  if (perform_ilu) {
    ...
    auto A = SparseUtils::BSRMat<double, 1, 1>(
        nnodes, nnodes, orig_nnzb, orig_rowPtr, orig_colPtr, nullptr);
    // A.perm = final_perm;
    // A.iperm = final_iperm; // this may delete perm here, may want to put in
    // ApplyPerm directly
    auto A2 =
        SparseUtils::BSRMatApplyPerm<double, 1>(A, final_perm, final_iperm);
    // int levFill = 3;
    int *levels;
