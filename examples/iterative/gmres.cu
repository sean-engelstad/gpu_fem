...
int main(void)
{
    ...
    double nrm_R;
    CHECK_CUBLAS(cublasDnrm2(cublasHandle, m, resid_ptr, 1, &nrm_R))
    ...
    for (int j = 0; j < m; j++) {
        ...
        cusparseDbsrsv2_solve(cusparseHandle, dir, trans_L, mb, nnzb, &a, descr_M,
            precond_vals, dp_rowp, dp_cols, block_dim, info_L,
            tmp2_ptr, tmp3_ptr, policy_L, pBuffer);

        // U^-1 * tmp3 => tmp2
        a = 1.0;
        cusparseDbsrsv2_solve(cusparseHandle, dir, trans_U, mb, nnzb, &a, descr_M,
            precond_vals, dp_rowp, dp_cols, block_dim, info_U,
            tmp3_ptr, tmp2_ptr, policy_U, pBuffer);
    ...
    for (int j = 0; j < m; j++) {
        ...
        CHECK_CUBLAS(cublasDaxpy(cublasHandle, nvars, &a, tmp1_ptr, 1, soln_ptr, 1));
    }
