...
#include "multigrid/prolongation/_unstruct_utils2.h"
...
class LockingAwareUnstructuredProlongation {
   public:
    ...
    void construct_nz_pattern() {
        ...
        int nodes_per_elem = kmat_bsr_data.nodes_per_elem;
        printf("nodes_per_elem %d\n", nodes_per_elem);
        if (nodes_per_elem == 4) {
            printf("unstruct grid maps\n");
            init_unstructured_grid_maps<T, Assembler, Basis, is_bsr, include_restrict>(
                fine_assembler, coarse_assembler, prolong_mat, restrict_mat, d_coarse_conn,
                d_n2e_ptr, d_n2e_elems, d_n2e_xis, ELEM_MAX);
            printf("\tdone with unstruct grid maps\n");
        } else if (nodes_per_elem == 9) {
            printf("unstructured grid v2\n");
            init_unstructured_grid_maps_v2<T, Assembler, Basis, is_bsr, include_restrict>(
                fine_assembler, coarse_assembler, prolong_mat, restrict_mat, d_coarse_conn,
                d_n2e_ptr, d_n2e_elems, d_n2e_xis, ELEM_MAX);
            printf("\tdone with unstructured grid v2\n");
        }
        ...
        // int *h_rowp = DeviceVec<int>(P_bsr_data.mb + 1,
        // P_bsr_data.rowp).createHostVec().getPtr(); int *h_cols = DeviceVec<int>(P_bsr_data.nnzb,
        // P_bsr_data.cols).createHostVec().getPtr(); for (int i = 0; i < P_bsr_data.mb; i++) {
        //     printf("construct nz pattern: h_prol row[%d]: ", i);
        ...
        d_coarse_ones = DeviceVec<T>(N_coarse).getPtr();
        d_coarse_weights = DeviceVec<T>(N_coarse).getPtr();
        d_fine_weights = DeviceVec<T>(N_fine).getPtr();
        d_fine_ones = DeviceVec<T>(N_fine).getPtr();
        k_vec_set<T><<<(N_fine + 31) / 32, 32>>>(N_fine, 1.0, d_fine_ones);
        k_vec_set<T><<<(N_coarse + 31) / 32, 32>>>(N_coarse, 1.0, d_coarse_ones);
    }
    ...
    void apply_kmat_fillin() {
        // get matrix filin P0 => A*P0, done on host and then update the device pointers (only
        // once)
        ...
        // need to add fc_elem map and fine and coarse elem conn in order to get correct elem
        // ind maps
        ...
        // P_bsr_data = BsrData(nnodes_fine, block_dim, AP_nnzb, d_P_rowp, d_P_cols,
        // d_fine_perm,
        //                      d_fine_iperm, false);
    ...
    void assemble_matrices() {
        ...
        // divide by row sums
        cudaMemset(d_fine_weights, 0.0, N_fine * sizeof(T));
        T a = 1.0, b = 0.0;
        int mb = P_bsr_data.mb, nb = P_bsr_data.nb;
        CHECK_CUSPARSE(cusparseDbsrmv(
            handle, CUSPARSE_DIRECTION_ROW, CUSPARSE_OPERATION_NON_TRANSPOSE, mb, nb, P_nnzb, &a,
            descr_P, d_P_vals, d_P_rowp, d_P_cols, block_dim, d_coarse_ones, &b, d_fine_weights));

        int nvals = P_nnzb * block_dim2;
        k_matrix_divide_row_sums<T>
            <<<(nvals + 31) / 32, 32>>>(P_nnzb, block_dim, d_P_rows, d_fine_weights, d_P_vals);
    }

    void update_after_smooth() {
        // compute coarse weights for nonlinear problems as P^T * ones => weights (row-sums of
        // P^T coarse nodes)
   ...
   private:
    ...
    int P_nnzb;
    T *d_coarse_weights, *d_fine_weights;
    T *d_fine_ones, *d_coarse_ones;
