...
#include "include/lock_prolong_v2.h"
...
void multigrid_solve(int nxe, double SR, int nsmooth, int ninnercyc, int nsmooth_mat, T omega, std::string cycle_type) {
    ...
    for (int i = 0; i < nlevels - 1; i++) {
        ...
        CHECK_CUDA(cudaDeviceSynchronize());
        printf("apply bcs rows\n");

        // int *h_fine_bcs = DeviceVec<int>(d_fine_bcs.getSize(), d_fine_bcs.getPtr()).createHostVec().getPtr();
        // printf("h_fine_bcs: ");
        // printVec<int>(d_fine_bcs.getSize(), h_fine_bcs);
