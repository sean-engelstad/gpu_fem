...
class MIG2CylinderAssembler:
    """
    Structured 2D IGA cylinder discretization on (x,s) for the MIG-MITC element.
    ...
    Field spaces:
      ...
      u   : (p2,p2) -> (nxe+2) x (nye+2)
      v   : (p2,p2) -> (nxe+2) x (nye+2)
    ...
    def __init__(
        ...
        # -----------------------------
        # Grid sizes (updated MIG-MITC spaces)
        # -----------------------------
        self.nx_w,   self.ny_w   = self.nxe + 2, self.nye + 2  # w   (p2,p2)
        self.nx_u,   self.ny_u   = self.nxe + 2, self.nye + 2  # u   (p2,p2)
        self.nx_v,   self.ny_v   = self.nxe + 2, self.nye + 2  # v   (p2,p2)
    ...
    def _build_block_patterns(self):
        ...
        self.wu_rowp,   self.wu_cols,   self.wu_nnz   = self._build_pattern_from_stencil(self.nw,   gw,  gu,  (-2, 2), (-2, 2), per)
        self.wv_rowp,   self.wv_cols,   self.wv_nnz   = self._build_pattern_from_stencil(self.nw,   gw,  gv,  (-2, 2), (-2, 2), per)
        self.wthx_rowp, self.wthx_cols, self.wthx_nnz = self._build_pattern_from_stencil(self.nw,   gw,  gtx, (-2, 1), (-2, 2), per)
        self.wthy_rowp, self.wthy_cols, self.wthy_nnz = self._build_pattern_from_stencil(self.nw,   gw,  gty, (-2, 2), (-2, 1), per)

        # u row
        self.uw_rowp,   self.uw_cols,   self.uw_nnz   = self._build_pattern_from_stencil(self.nu_d, gu,  gw,  (-2, 2), (-2, 2), per)
        self.uu_rowp,   self.uu_cols,   self.uu_nnz   = self._build_pattern_from_stencil(self.nu_d, gu,  gu,  (-2, 2), (-2, 2), per)
        self.uv_rowp,   self.uv_cols,   self.uv_nnz   = self._build_pattern_from_stencil(self.nu_d, gu,  gv,  (-2, 2), (-2, 2), per)
        self.uthx_rowp, self.uthx_cols, self.uthx_nnz = self._build_pattern_from_stencil(self.nu_d, gu,  gtx, (-2, 1), (-2, 2), per)
        self.uthy_rowp, self.uthy_cols, self.uthy_nnz = self._build_pattern_from_stencil(self.nu_d, gu,  gty, (-2, 2), (-2, 1), per)

        # v row
        self.vw_rowp,   self.vw_cols,   self.vw_nnz   = self._build_pattern_from_stencil(self.nv,   gv,  gw,  (-2, 2), (-2, 2), per)
        self.vu_rowp,   self.vu_cols,   self.vu_nnz   = self._build_pattern_from_stencil(self.nv,   gv,  gu,  (-2, 2), (-2, 2), per)
        self.vv_rowp,   self.vv_cols,   self.vv_nnz   = self._build_pattern_from_stencil(self.nv,   gv,  gv,  (-2, 2), (-2, 2), per)
        self.vthx_rowp, self.vthx_cols, self.vthx_nnz = self._build_pattern_from_stencil(self.nv,   gv,  gtx, (-2, 1), (-2, 2), per)
        self.vthy_rowp, self.vthy_cols, self.vthy_nnz = self._build_pattern_from_stencil(self.nv,   gv,  gty, (-2, 2), (-2, 1), per)

        # thx row
        self.thxw_rowp,   self.thxw_cols,   self.thxw_nnz   = self._build_pattern_from_stencil(self.nthx, gtx, gw,  (-1, 2), (-2, 2), per)
        self.thxu_rowp,   self.thxu_cols,   self.thxu_nnz   = self._build_pattern_from_stencil(self.nthx, gtx, gu,  (-1, 2), (-2, 2), per)
        self.thxv_rowp,   self.thxv_cols,   self.thxv_nnz   = self._build_pattern_from_stencil(self.nthx, gtx, gv,  (-1, 2), (-2, 2), per)
        ...
        self.thyu_rowp,   self.thyu_cols,   self.thyu_nnz   = self._build_pattern_from_stencil(self.nthy, gty, gu,  (-2, 2), (-1, 2), per)
        self.thyv_rowp,   self.thyv_cols,   self.thyv_nnz   = self._build_pattern_from_stencil(self.nthy, gty, gv,  (-2, 2), (-1, 2), per)
    ...
    def restrict_defect(self, fine_defect: np.ndarray):
        nxe_c = self.nxe // 2
        nye_c = self.nye // 2
        return self.element.restrict(fine_defect, nxe_c, nye_c)
