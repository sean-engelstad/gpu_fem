...
int main(void) {
  ...
  static constexpr int MELD_NN = 64; // 32 (may be too small at 32)
  ...
  using Transfer = MELD<T, MELD_NN, linear_meld>;
  ...
  int sym = -1;
  Transfer transfer = Transfer(xs0, xa0, beta, MELD_NN, sym, Hreg);
  transfer.initialize();
