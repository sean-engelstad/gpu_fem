...
class ShellQuadBasis {
   public:
    ...
    __HOST_DEVICE__ static T lagrangeLobatto2DLight(const int i, const T xi, const T eta) {
        // compute N_i(u) shape function values at each node
        if constexpr (order == 2) {
            return lagrangeLobatto1DLight(i % 2, xi) * lagrangeLobatto1DLight(i / 2, eta);
        }
    }  // end of lagrangeLobatto2D
    ...
    template <int DIREC>
    __HOST_DEVICE__ static T lagrangeLobatto2DGradLight(const int i, const T xi, const T eta) {
        // compute N_i(u) shape function values at each node
        if constexpr (order == 2) {
            if constexpr (DIREC == 0) {  // XI direction
                return lagrangeLobatto1DGradLight(i % 2, xi) * lagrangeLobatto1DLight(i / 2, eta);
            } else {  // ETA direction
                return lagrangeLobatto1DLight(i % 2, xi) * lagrangeLobatto1DGradLight(i / 2, eta);
            }
        }
    }  // end of lagrangeLobatto2DGradLight
    ...
    template <int vars_per_node, int num_fields>
    __HOST_DEVICE__ static void interpFieldsLight(const T pt[], const T values[], T field[]) {
#pragma unroll
        for (int ifield = 0; ifield < num_fields; ifield++) {
            field[ifield] = 0.0;
        }
...
#pragma unroll
        for (int inode = 0; inode < num_nodes; inode++) {
            T Ni = lagrangeLobatto2DLight(inode, pt[0], pt[1]);
#pragma unroll
            for (int ifield = 0; ifield < num_fields; ifield++) {
                field[ifield] += Ni * values[inode * vars_per_node + ifield];
            }
        }
    }  // end of interpFields method
    ...
    template <int vars_per_node, int num_fields>
    __HOST_DEVICE__ static void interpFieldsGradLight(const T pt[], const T values[], T dxi[],
                                                      T deta[]) {
#pragma unroll
        for (int ifield = 0; ifield < num_fields; ifield++) {
            dxi[ifield] = 0.0;
...
#pragma unroll
        for (int inode = 0; inode < num_nodes; inode++) {
            T Nj_xi = lagrangeLobatto2DGradLight<0>(inode, pt[0], pt[1]);
            T Nj_eta = lagrangeLobatto2DGradLight<1>(inode, pt[0], pt[1]);
#pragma unroll
            for (int ifield = 0; ifield < num_fields; ifield++) {
                dxi[ifield] += Nj_xi * values[inode * vars_per_node + ifield];
                deta[ifield] += Nj_eta * values[inode * vars_per_node + ifield];
            }
        }
    }  // end of interpFieldsGrad method

    template <int vars_per_node, int num_fields, int direc>
    __HOST_DEVICE__ static void interpFieldsGradLight(const T pt[], const T values[], T dfield[]) {
#pragma unroll
        for (int ifield = 0; ifield < num_fields; ifield++) {
            dfield[ifield] = 0.0;
        }
...
#pragma unroll
        for (int inode = 0; inode < num_nodes; inode++) {
            T Nj_xi = lagrangeLobatto2DGradLight<direc>(inode, pt[0], pt[1]);
#pragma unroll
            for (int ifield = 0; ifield < num_fields; ifield++) {
                dfield[ifield] += Nj_xi * values[inode * vars_per_node + ifield];
            }
        }
    }  // end of interpFieldsGrad method

    template <int vpn1, int vpn2, int num_fields, int direc1, int direc2>
...
#pragma unroll
        ...
        return out;
    }  // end of interpFieldsDotProduct method

    template <int vpn1, int vpn2, int num_fields, int direc1, int direc2>
    __HOST_DEVICE__ static void interpFieldsGradDotProduct_LeftSens(const T &scale, const T pt[],
                                                                    T vals1_out[],
                                                                    const T vals2[]) {
        T out = 0.0;
        // #pragma unroll
        for (int inode = 0; inode < num_nodes; inode++) {
            ...
            // #pragma unroll
            for (int jnode = 0; jnode < num_nodes; jnode++) {
                T Nj = lagrangeLobatto2DGradLight<direc2>(jnode, pt[0], pt[1]);
                // #pragma unroll
                for (int ifield = 0; ifield < num_fields; ifield++) {
                    vals1_out[inode * vpn1 + ifield] +=
                        Ni * Nj * vals2[jnode * vpn2 + ifield] * scale;
                }
            }
        }
    }  // end of interpFieldsDotProduct method
    ...
    template <int vpn1, int vpn2, int num_fields, int direc1, int direc2>
    __HOST_DEVICE__ static void interpFieldsGradDotProduct_RightSens(const T &scale, const T pt[],
                                                                     const T vals1[],
                                                                     T vals2_out[]) {
// this version doesn't parallelize and reuse registers well (high register ct)
#pragma unroll
        for (int jnode = 0; jnode < num_nodes; jnode++) {
            T Nj = scale * lagrangeLobatto2DGradLight<direc2>(jnode, pt[0], pt[1]);
            {
                // #pragma unroll
                for (int inode = 0; inode < num_nodes; inode++) {
                    T Ni = Nj * lagrangeLobatto2DGradLight<direc1>(inode, pt[0], pt[1]);

                    // #pragma unroll
                    for (int ifield = 0; ifield < num_fields; ifield++) {
                        vals2_out[jnode * vpn2 + ifield] += Ni * vals1[inode * vpn1 + ifield];
                    }
                }
            }
        }

        // try new version that stores some results *get same answer as above with better storage
        // between for loops)
        //         T Ni[num_nodes], Nj[num_nodes];
        // #pragma unroll
        //         for (int inode = 0; inode < num_nodes; inode++) {
        //             Ni[inode] = lagrangeLobatto2DGradLight<direc1>(inode, pt[0], pt[1]);
        //             Nj[inode] = lagrangeLobatto2DGradLight<direc2>(inode, pt[0], pt[1]);
        //         }

        // #pragma unroll
        //         for (int jnode = 0; jnode < num_nodes; jnode++) {
        //             T _Nj = Nj[jnode] * scale;
        //             {
        //                 for (int inode = 0; inode < num_nodes; inode++) {
        //                     T _Ni = _Nj * Ni[inode];
        //                     for (int ifield = 0; ifield < num_fields; ifield++) {
        //                         vals2_out[jnode * vpn2 + ifield] += _Ni * vals1[inode * vpn1 +
        //                         ifield];
        //                     }
        //                 }
        //             }
    ...
    template <int vpn1, int vpn2, int num_fields, int direc1>
    __HOST_DEVICE__ static T interpFieldsMixedDotProduct(const T pt[], const T vals1[],
                                                         const T vals2[]) {
        /* first direction is grad, second isn't*/
        T out = 0.0;
#pragma unroll
        for (int inode = 0; inode < num_nodes; inode++) {
            T Ni = lagrangeLobatto2DGradLight<direc1>(inode, pt[0], pt[1]);
            // #pragma unroll
            for (int jnode = 0; jnode < num_nodes; jnode++) {
                T Nj = Ni * lagrangeLobatto2DLight(jnode, pt[0], pt[1]);
                // #pragma unroll
                for (int ifield = 0; ifield < num_fields; ifield++) {
                    out += Nj * vals1[inode * vpn1 + ifield] * vals2[jnode * vpn2 + ifield];
                }
    ...
    template <int vpn1, int vpn2, int num_fields, int direc1>
    __HOST_DEVICE__ static void interpFieldsMixedDotProduct_Sens(const T &scale, const T pt[],
                                                                 T vals1_out[], const T vals2[]) {
        /* first direction is grad, second isn't*/
#pragma unroll
        for (int inode = 0; inode < num_nodes; inode++) {
            T Ni = scale * lagrangeLobatto2DGradLight<direc1>(inode, pt[0], pt[1]);
            {
                // #pragma unroll
                for (int jnode = 0; jnode < num_nodes; jnode++) {
                    T Nj = Ni * lagrangeLobatto2DLight(jnode, pt[0], pt[1]);
                    // #pragma unroll
                    for (int ifield = 0; ifield < num_fields; ifield++) {
                        vals1_out[inode * vpn1 + ifield] += Nj * vals2[jnode * vpn2 + ifield];
                    }
                }
            }
        }
    ...
    __HOST_DEVICE__ static void interpFieldsGradTransposeLight(const T pt[], const T dxi_bar[],
        ...
        //             for (int inode = 0; inode < num_nodes; inode++) {
        //                 values_bar[inode * vars_per_node + ifield] +=
        //                     dxi_bar[ifield] * lagrangeLobatto2DGradLight<0>(inode, pt[0],
        //                     pt[1])
        //                     + deta_bar[ifield] * lagrangeLobatto2DGradLight<1>(inode, pt[0],
        //                     pt[1]);
        //             }
        //         }
...
#pragma unroll
            for (int ifield = 0; ifield < num_fields; ifield++) {
                values_bar[inode * vars_per_node + ifield] +=
                    dxi_bar[ifield] * Nj_xi + deta_bar[ifield] * Nj_eta;
            }
        }
    }  // end of interpFieldsGrad method
...
template <typename T, class Basis, int vars_per_node, bool is_nonlinear, int strain>
__DEVICE__ __noinline__ static void computeInterpTyingStrainSens(T quad_pt[], T &gtyb,
                                                                 const T xpts[], const T vars[],
                                                                 T res[]) {
    if constexpr (strain == 0) {
        // reverse of gty[0] aka g11 => res
        constexpr int num_tying_g11 = Basis::num_tying_points(0);
...
#pragma unroll
        for (int itying = 0; itying < num_tying_g11; itying++) {
            // first entry for g11 reverse
            {
                ...
                T etyb = gtyb * N_g11[itying];  // reversed interp

                // Interpolate the field value
                T Uxi[3], Xxi[3];
                {
                    T Ueta[3], Xeta[3];
                    Basis::template interpFieldsGradLight<3, 3>(pt, xpts, Xxi, Xeta);
                    Basis::template interpFieldsGradLight<vars_per_node, 3>(pt, vars, Uxi, Ueta);
                }

                A2D::Vec<T, 3> Uxi_bar, zero;
                A2D::VecAddCore<T, 3>(etyb, Xxi, Uxi_bar.get_data());

                // nonlinear g11 strain term backprop
                if constexpr (is_nonlinear) {
                    // recall forward analysis nonlinear g11 term:
                    // ety[offset + itying] += 0.5 * A2D::VecDotCore<T, 3>(Uxi,
                    // Uxi);
    ...
    if constexpr (strain == 1) {
        // reverse of gty[0] aka g11 => res
        constexpr int num_tying_g22 = Basis::num_tying_points(1);
        T N_g22[num_tying_g22];
        // Basis::template getTyingInterp<1>(quad_pt, N_g22);
        N_g22[0] = 0.5 * (1 - quad_pt[0]);
        N_g22[1] = 0.5 * (1 + quad_pt[0]);

        {  // g22 first iteration
            T pt[2];
            // Basis::template getTyingPoint<1>(0, pt);
            pt[0] = -1.0;
            pt[1] = 0.0;

            T etyb = gtyb * N_g22[0];  // reversed interp
            ...
            A2D::VecAddCore<T, 3>(etyb, Xeta, Ueta_bar.get_data());

            // nonlinear g22 strain term backprop
            if constexpr (is_nonlinear) {
                // recall forward analysis nonlinear g22 term:
                // ety[offset + itying] += 0.5 * A2D::VecDotCore<T, 3>(Ueta,
                // Ueta);
                A2D::VecAddCore<T, 3>(etyb, Ueta, Ueta_bar.get_data());
            }

            Basis::template interpFieldsGradTransposeLight<vars_per_node, 3>(
                pt, zero.get_data(), Ueta_bar.get_data(), res);
        }
    }  // end of g22 reverse
}
    ...
    {  // scope block for backprop of u0x_bar, u1x_bar to res
        ...
        if constexpr (back_to_director) {
            ...
            // scope for u0d_barT
            {
                ...
                // transfer back to u0xi, u0eta, d0 bar (with transpose so columns now available
                // in rows)
                Basis::template interpFieldsTranspose<3, 3>(pt, &u0d_barT[6], d_bar);
