...
#include "domdec/nl_bddc_interface.h"
...
class Nonlinear_BDDCLU_PlateSolver {
   public:
    using T = double;
    ...
    using StructSolver = TACSNLBddcInterface<T, Assembler, BDDC_WRAPPER, NL, BDDC>;

    using DMass = Mass<T, DeviceVec>;
    using DKSFail = KSFailure<T, DeviceVec>;

    Nonlinear_BDDCLU_PlateSolver(double rhoKS = 100.0, double safety_factor = 1.5,
        ...
        thick_ = 1.0 / SR_;
        mag_ = load_mag_;
        nxe_per_comp_ = nxe_ / nx_comp_;
        nye_per_comp_ = nye / ny_comp_;

        CHECK_CUBLAS(cublasCreate(&cublasHandle));
        CHECK_CUSPARSE(cusparseCreate(&cusparseHandle));

        // ------------------------------------------------------------------
        // 1) Single fine assembler, exactly like Linear_BDDCLU_PlateSolver
        // ------------------------------------------------------------------
        ...
        bddc->add_subdomain_fext(load_functor, mag_, in_plane_frac_);
        printf("\tdone with add subdomain fext\n");

        printf("add glob vec fext\n");
        assembler.add_fext_fast(load_functor, mag_, loads, in_plane_frac_);
        ...
        solver =
            new StructSolver(cublasHandle, nl_solver, krylov_bddc_wrapper, bddc, assembler, print_);
   ...
   private:
    ...
    ObliqueShearSineLoad2<T> load_functor;
    SolverOptions opts;
