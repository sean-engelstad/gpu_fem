...
class Connectivity3D {
 ...
 public:
  ...
  int get_tets(const Tetrahedron **ts) const {
    if (ts) {
      *ts = tets;
    }
    return num_tets;
  }
  int get_hexs(const Hexahedron **hx) const {
    if (hx) {
      *hx = hexs;
    }
    return num_hexs;
  }
  int get_pyrds(const Pyramid **pys) const {
    if (pys) {
      *pys = pyrds;
    }
    return num_pyrds;
  }
  int get_wedges(const Wedge **wdgs) const {
    if (wdgs) {
      *wdgs = wedges;
    }
    return num_wedges;
