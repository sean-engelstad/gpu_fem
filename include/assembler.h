...
class ElementAssembler {
 ...
 void add_jacobian(T *res, T *mat) {
  ...
  // input is either a device array when USE_GPU or a host array if not USE_GPU
  #ifdef USE_GPU
    ElemGroup::template add_jacobian<Data>(num_elements, d_geo_conn, d_vars_conn, d_xpts, d_vars, d_physData, res, mat);
  #else // USE_GPU
    ElemGroup::template add_jacobian<Data>(num_elements, h_geo_conn, h_vars_conn, h_xpts, h_vars, h_physData, res, mat);
  #endif // USE_GPU
