...
void perform_ichol0_factorization(cusparseHandle_t handle, cusparseMatDescr_t &descr_L,
    ...
    // cusparseSetMatType(descr_M, CUSPARSE_MATRIX_TYPE_SYMMETRIC);
    cusparseSetMatType(descr_M, CUSPARSE_MATRIX_TYPE_GENERAL);  // can't support SYM here
    cusparseSetMatFillMode(descr_M, CUSPARSE_FILL_MODE_LOWER);
    cusparseSetMatDiagType(descr_M, CUSPARSE_DIAG_TYPE_NON_UNIT);
    cusparseCreateBsrilu02Info(&info_M);

    // init L matrix objects (for triangular solve)
    cusparseCreateMatDescr(&descr_L);
    cusparseSetMatIndexBase(descr_L, CUSPARSE_INDEX_BASE_ZERO);
    // cusparseSetMatType(descr_L, CUSPARSE_MATRIX_TYPE_SYMMETRIC);
    cusparseSetMatType(descr_L, CUSPARSE_MATRIX_TYPE_GENERAL);  // can't support SYM here
