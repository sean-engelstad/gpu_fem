...
__HOST_DEVICE__ T ShellComputeDispGradSens(
    ...
    const T u0x_bar[], const T u1x_bar[], T res[]) {
  // 75 floats used in this method
  // up to 217 floats here (but then goes back down to 169 floats after out of
  // scope)
  ...
  // reverse u0x_bar => u0d_bar (SPE)
  // u0d_bar = T^t * Xdinv^-t * u0x_bar * T
  T u0d_bar[9];
  A2D::MatMatMultCore3x3<T, A2D::MatOp::TRANSPOSE>(XdinvT, u0x_bar, temp);
  A2D::MatMatMultCore3x3<T>(temp, Tmat, u0d_bar);
