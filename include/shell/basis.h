...
class ShellQuadBasis {
 public:
  ...
  __HOST_DEVICE__ static constexpr int32_t num_tying_points(int icomp) {
    ...
    int32_t _num_tying_points[] = {mitcLevel2, mitcLevel3, mitcLevel2, mitcLevel2, mitcLevel2};
    return _num_tying_points[icomp];
  }

  __HOST_DEVICE__ static constexpr int32_t tying_point_offsets(int icomp) {
    // g11, g12, g13, g22, g23, g33=0 (not included g33)
    int32_t _tying_point_offsets[] = {0, mitcLevel2, mitcLevel2+mitcLevel3, 2*mitcLevel2 + mitcLevel3, 3*mitcLevel2 + mitcLevel3};
 ...
 __HOST_DEVICE__ static void lagrangeLobatto2DGrad(const T xi, const T eta, T *N, T *dNdxi, T *dNdeta) {
    // compute N_i(u) shape function values at each node
    if constexpr (order == 2) {
        T na[order], nb[order];
...
__HOST_DEVICE__ void ShellComputeNodeNormals(const T Xpts[], T fn[], T Xdn[]) {
    ...
    for (int inode = 0; inode < Basis::num_nodes; inode++) {
        ...
        // ----------------------------------------------------
        // // be careful this doesn't cause warp divergence on GPU (prob not but just be careful)
        // if (fnorm) {
        //     fnorm[inode] = norm;
        // }
        // if (Xdn) {
        //     Basis::template assembleFrame(dXdxi, dXdeta, &fn[3*inode], &Xdn[9*inode]);
        // }
    }
}
...
__HOST_DEVICE__ T ShellComputeDispGrad(const T pt[], const T xpts[],
    const T vars[], const T fn[], const T d[],
    const T Xxi[], const T Xeta[], const T n0[], const T Tmat[],
    T XdinvT[], T u0x[], T u1x[]) {

    // 75 floats used in this method
    // up to 217 floats here (but then goes back down to 169 floats after out of scope)
...
__HOST_DEVICE__ static void interpTyingStrain(const T pt[], const T ety[], T gty[]) {
    ...
    // interp the final tying strain {g11, g12, g13, g22, g23} at this point with g33 = 0 also
    int32_t offset;
