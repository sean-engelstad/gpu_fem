...
class ShellCompositeStiffenedPanelData
{
public:
    ...
    __HOST_DEVICE__ ShellCompositeStiffenedPanelData(T E_, T nu_, T thick_, T refAxis_[], T tOffset_ = 0.0) : E(E_), nu(nu_), thick(thick_), tOffset(tOffset_)
    {
    ...
    template <bool U = has_ref_axis, typename std::enable_if<!U, int>::type = 0>
    __HOST_DEVICE__ ShellCompositeStiffenedPanelData(T E_, T nu_, T thick_, T tOffset_ = 0.0) : E(E_), nu(nu_), thick(thick_), tOffset(tOffset_) {}
    ...
    // TODO : add thickness offset (rarely used but to be complete.. can allow B matrix to be nonzero)
