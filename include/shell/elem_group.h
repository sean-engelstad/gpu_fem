...
// some if constexpr stuff on type of Basis?
    ...
    __HOST_DEVICE__ static void add_element_quadpt_energy(
        ...
        const T vars[dof_per_elem], const Data physData, T &Uelem)
    {
        ...
        // ------------------------------------------------
        {
            ...
            // compute directors
            T d[3 * num_nodes];
            Director::template computeDirector<vars_per_node, num_nodes>(vars,
                                                                         fn, d);
            ...
            Phys::template computeStrainEnergy<T>(physData, scale, u0x, u1x,
                                                  e0ty, et, _Uelem);
...

    {
        ...
        // ------------------------------------------------
        {
            ...
            // compute energy + energy-dispGrad sensitivites with physics
            Phys::template computeWeakRes<T>(physData, scale, u0x, u1x, e0ty,
                                             et);
        ...
        ShellComputeDispGradSens<T, vars_per_node, Basis, Data>(
            ...
            u1x.bvalue().get_data(), e0ty.bvalue(), res, d_bar.get_data(),
            ety_bar.get_data());
        ...
        Phys::template computeTyingStrainSens<Basis>(
            xpts, fn, ety_bar.get_data(), res, d_bar.get_data());
    ...
    __HOST_DEVICE__ static void add_element_quadpt_jacobian_col(
        const bool active_thread, const int iquad, const int ivar,
        const T xpts[xpts_per_elem], const T vars[dof_per_elem],
        const Data physData, T res[dof_per_elem], T matCol[dof_per_elem])
    {
        // keep in mind max of ~256 floats on single thread
        ...
        p_vars[ivar] =
            1.0; // p_vars is unit vector for current column to compute
        {
            ShellComputeDrillStrainHfwd<T, vars_per_node, Data, Basis,
                                        Director>(pt, physData.refAxis, xpts,
                                                  p_vars.get_data(), fn,
                                                  et.pvalue().get_data());

            T p_d[3 * num_nodes];
            Director::template computeDirectorHfwd<vars_per_node, num_nodes>(
                p_vars.get_data(), fn, p_d);

            T p_ety[Basis::num_all_tying_points];
            Phys::template computeTyingStrainHfwd<Basis>(
                xpts, fn, p_vars.get_data(), p_d, p_ety);
        ...
        // ---------------------
        Phys::template computeWeakJacobianCol<T>(physData, scale, u0x, u1x,
                                                 e0ty, et);
        // ---------------------
        ...
        {
            A2D::Vec<T, Basis::num_all_tying_points>
                ety_bar;                      // zeroes out on init
            ...
            ShellComputeDrillStrainSens<T, vars_per_node, Data, Basis,
                                        Director>(pt, physData.refAxis, xpts,
                                                  vars, fn,
        ...
        // hreverse (2nd order derivs)
        {
            ...
            ShellComputeDispGradHrev<T, vars_per_node, Basis, Data>(
                ...
                u1x.hvalue().get_data(), e0ty.hvalue(), res, d_hat.get_data(),
                ety_hat.get_data());

            Phys::template computeTyingStrainHrev<Basis>(
                xpts, fn, ety_hat.get_data(), matCol, d_hat.get_data());

            Director::template computeDirectorHrev<vars_per_node, num_nodes>(
                fn, d_hat.get_data(), matCol);

            ShellComputeDrillStrainHrev<T, vars_per_node, Data, Basis,
                                        Director>(
                pt, physData.refAxis, xpts, vars, fn, et.hvalue().get_data(),
                matCol);
...

    {
        ...
        // ------------------------------------------------
        {
            ...
            T detXd = ShellComputeDispGrad<T, vars_per_node, Basis, Data>(
                pt, physData.refAxis, xpts, vars, fn, d, ety,
                u0x.get_data(), u1x.get_data(), e0ty.get_data());

            // get the scale for disp grad sens of the energy
            T scale = detXd * weight;

            // compute energy + energy-dispGrad sensitivites with physics
            Phys::template computeQuadptStresses<T>(physData, scale, u0x, u1x, e0ty,
                                                    et, stresses);
    ...
    template <class Data>
    __HOST_DEVICE__ static void
    add_element_quadpt_gmat_col(const bool active_thread, const int iquad,
                                const int ivar, const T xpts[xpts_per_elem],
                                const T vars0[dof_per_elem],
                                const T path[dof_per_elem], const Data physData,
                                T matCol[dof_per_elem])
    {
            ...
            1.0; // p_vars is unit vector for current column to compute
        {
            ...
            ShellComputeDrillStrain<T, vars_per_node, Data, Basis, Director>(
                pt, physData.refAxis, xpts, p_vars.get_data(), fn,
                et.pvalue().get_data());
            ...
            T p_ety[Basis::num_all_tying_points];
            Phys::template computeTyingStrain<Basis>(
                xpts, fn, p_vars.get_data(), p_d, p_ety);
        ...
        // residual backprop section (1st order derivs)
        {
            ...
            ShellComputeDispGradSens<T, vars_per_node, Basis, Data>(
                pt, physData.refAxis, xpts, vars, fn, u0x.bvalue().get_data(),
                u1x.bvalue().get_data(), e0ty.bvalue(), matCol,
                d_bar.get_data(), ety_bar);

            // drill strain sens
            ShellComputeDrillStrainSens<T, vars_per_node, Data, Basis,
                                        Director>(
                pt, physData.refAxis, xpts, vars, fn, et.bvalue().get_data(),
                matCol);

            // backprop tying strain sens ety_bar to d_bar and res
            Phys::template computeTyingStrainSens<Basis>(
                xpts, fn, ety_bar, matCol, d_bar.get_data());

            // directors back to residuals
            Director::template computeDirectorSens<vars_per_node, num_nodes>(
                fn, d_bar.get_data(), matCol);
        ...
        // -----------------------------------------------------
        {
            ...
            ShellComputeDispGradSens<T, vars_per_node, Basis, Data>(
                pt, physData.refAxis, xpts, vars, fn, u0x.hvalue().get_data(),
                u1x.hvalue().get_data(), e0ty.hvalue(), matCol,
                d_hbar.get_data(), ety_hbar);
            ...
            Phys::template computeTyingStrainSens<Basis>(
                xpts, fn, ety_hbar, matCol, d_hbar.get_data());

            Director::template computeDirectorSens<vars_per_node, num_nodes>(
                fn, d_hbar.get_data(), matCol);
