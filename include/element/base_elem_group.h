...
class BaseElementGroup {
   public:
    ...
    static void add_residual_cpu(const int32_t num_elements, const Vec<int32_t> &geo_conn,
        ...
        for (int ielem = 0; ielem < num_elements; ielem++) {
            ...
            xpts.getElementValues(Geo::spatial_dim, Geo::num_nodes, geo_elem_conn, &elem_xpts[0]);

            T elem_vars[vars_per_elem];
            const int32_t *vars_elem_conn = &_vars_conn[ielem * Basis::num_nodes];
            vars.getElementValues(Phys::vars_per_node, Basis::num_nodes, vars_elem_conn,
                                  &elem_vars[0]);
            ...
            res.addElementValues(1.0, Phys::vars_per_node, Basis::num_nodes, vars_elem_conn,
                                 elem_res);
    ...
    static void add_jacobian_cpu(int32_t vars_num_nodes, int32_t num_elements,
        ...
        for (int ielem = 0; ielem < num_elements; ielem++) {
            ...
            const int32_t *geo_elem_conn = &_geo_conn[ielem * Geo::num_nodes];
            xpts.getElementValues(Geo::spatial_dim, Geo::num_nodes, geo_elem_conn, elem_xpts);

            T elem_vars[vars_per_elem];
            const int32_t *vars_elem_conn = &_vars_conn[ielem * Basis::num_nodes];
            vars.getElementValues(Phys::vars_per_node, Basis::num_nodes, vars_elem_conn, elem_vars);
            ...
            for (int ideriv = 0; ideriv < vars_per_elem; ideriv++) {
                ...
                for (int iquad = 0; iquad < Quadrature::num_quad_pts; iquad++) {
                    Derived::template add_element_quadpt_jacobian_col<Data>(
                        true, iquad, ideriv, elem_xpts, elem_vars, elem_physData, elem_res,
            ...
            res.addElementValues(1.0 / vars_per_elem, Phys::vars_per_node, Basis::num_nodes,
                                 vars_elem_conn, elem_res);
            mat.addElementMatrixValues(1.0, ielem, Phys::vars_per_node, Basis::num_nodes,
