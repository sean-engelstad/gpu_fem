...
public:
  ...
  __HOST__ void scale(T myscale) { values.scale(myscale); }
  ...
  void copyValuesTo(BsrMat<Vec> mat) {
    ...
    // launch kernel to apply BCs to the full matrix
    copy_mat_values_kernel<T><<<grid, block>>>(nnodes, block_dim, rowp, cols,
                                               vals, t_rowp, t_cols, t_vals);
    CHECK_CUDA(cudaDeviceSynchronize());
#endif
  }
  ...
  void addElementMatrixValues(const T scale, const int ielem,
    ...
    for (int elem_block = 0; elem_block < blocks_per_elem; elem_block++) {
      ...
      int istart =
          nnz_per_block * elem_ind_map[blocks_per_elem * ielem + elem_block];
      T *val = &valPtr[istart];
