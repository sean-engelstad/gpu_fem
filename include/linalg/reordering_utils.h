...
// q-ordering (writing it myself), https://arc.aiaa.org/doi/epdf/10.2514/6.2020-3022
// how to apply ILU(k) from below RCM reorderings (reverrse cuthill-mckee)?
// here's how to implement q-ordering
// 1) reorder the CSR data structure with bandwidth minimizing step like RCM
// 2) compute the bandwidth of the new reordered structure
// 3) random reodering reorder every groups of rows with #rows=1/p * bandwidth where p = prune width
//      normal prune widths are 2,1,1/2,1/4 (lower prune width results in more nnz but more stable convergence of iterative solvers like GMRES)
