...
class PlateProlongation {
   ...
   public:
    static void prolongate(int nelems_fine, const int *d_fine_elem_conn, int *d_coarse_iperm,
                           int *d_fine_iperm, DeviceVec<T> coarse_soln_in, DeviceVec<T> dx_fine,
                           int *d_int_temp) {
        ...
        // one cool thing is that each fine element only touches one coarse node, that's the basis
        // of my algorithm here.. other option would be to use fine rowp + cols for adj nodes of
        // each coarse node (but would also need fine iperm then..) call even then odd elems ->
        // cause otherwise even + odd elems share coarse node and this results in bank conflicts..
        k_plate_prolongate<T>
            <<<grid, block>>>(nxe_coarse, nxe_fine, nelems_fine, d_fine_elem_conn, d_coarse_iperm,
                              d_fine_iperm, coarse_soln_in.getPtr(), dx_fine.getPtr(), d_int_temp);
    ...
    static void restrict_defect(int nelems_fine, const int *d_fine_elem_conn, int *d_coarse_iperm,
                                int *d_fine_iperm, DeviceVec<T> fine_defect_in,
                                DeviceVec<T> coarse_defect_out, int *d_int_temp) {
        // zero temp so we can store dx in it
        int N_coarse =
        ...
        // each coarse node (but would also need fine iperm then..) call even then odd elems ->
        // cause otherwise even + odd elems share coarse node and this results in bank conflicts..
        k_plate_restrict<T><<<grid, block>>>(nxe_coarse, nxe_fine, nelems_fine, d_fine_elem_conn,
                                             d_coarse_iperm, d_fine_iperm, fine_defect_in.getPtr(),
                                             coarse_defect_out.getPtr(), d_int_temp);

        // now normalize by the weights so partition of unity remains
        int nblock2 = (N_coarse + 31) / 32;
        dim3 grid2(nblock2);
        k_vec_normalize<T><<<grid2, block>>>(N_coarse, coarse_defect_out.getPtr(), d_int_temp);
