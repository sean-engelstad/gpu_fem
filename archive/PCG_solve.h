...
void iterative_PCG_solve(BsrMat<DeviceVec<T>> mat, DeviceVec<T> rhs,
    ...
    CHECK_CUSPARSE(cusparseDbsrilu02(cusparseHandle, CUSPARSE_DIRECTION_ROW, mb,
                                     nnzb, descr, d_values, d_rowPtr, d_colPtr,
                                     blockDim, iluInfo, policy, buffer));
    ...
    CHECK_CUSPARSE(cusparseCreateBsr(&matA, m, m, nnz, d_A_rows, d_A_columns,
                                     ...
                                     CUSPARSE_INDEX_32I, baseIdx, CUDA_R_64F))
    // L
    ...
    CHECK_CUSPARSE(cusparseSpMatSetAttribute(
        matL, CUSPARSE_SPMAT_DIAG_TYPE, &diag_non_unit, sizeof(diag_non_unit)))

    // resources,
    // https://
    //
    // docs.nvidia.com / cuda / cuda - samples / index.html #cusolversp -
    //     linear - solver - % 5B / url %
    //                           5D
    //                           // github repo #1 for PCG,
    //                           //
    //                           https
    //     : // github.com/NVIDIA/CUDALibrarySamples/tree/master/cuSPARSE/cg
    //       // github repo #2 for PCG,
    //       // https://
    //       //
    //       github.com / NVIDIA /
    //                           cuda -
    //     samples / tree / master / Samples / 4_CUDA_Libraries /
    //         conjugateGradientPrecond

    printf("CG loop:\n");
    _gpu_CG(cublasHandle, cusparseHandle, m, matA, matL, d_rhs, d_soln, d_resid,
